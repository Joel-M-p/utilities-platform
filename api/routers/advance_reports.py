...
def api_management_fee_invoice(property_id: int = None, current_user: dict = Depends(require_staff)):
    ...
    try:
        ...
        cursor.execute(
            ...
            "WHERE tx.amount > 0 AND tx.transaction_type IN ('WALLET_TOPUP', 'RENT_PAYMENT', 'UTILITY_PAYMENT')" + scope_where,
            params
        )
...
def api_bank_reconciliation(property_id: int = None, current_user: dict = Depends(require_staff)):
    ...
    try:
        ...
        cursor.execute("SELECT COALESCE(SUM(tx.amount), 0)" + scope_join + "WHERE tx.amount > 0" + scope_where, params)
        total_in = float(cursor.fetchone()[0])
        cursor.execute("SELECT COALESCE(SUM(ABS(tx.amount)), 0)" + scope_join + "WHERE tx.amount < 0" + scope_where, params)
...
def api_three_month_stats(property_id: int = None, current_user: dict = Depends(verify_token)):
    ...
    try:
        ...
        query = """
            ...
            JOIN wallets w ON tx.wallet_id = w.id
            JOIN tenants tn ON w.tenant_id = tn.id
            WHERE tx.amount < 0 AND tx.created_at > NOW() - INTERVAL '3 months'
        """
        if scope_id:
