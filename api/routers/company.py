...
@router.get("/company/", dependencies=[Depends(verify_token)])
def get_company_settings():
    ...
    try:
        ...
        return {
            ...
            "vat_percent": float(row[6]) if row[6] is not None else 15.0
        }
...
@router.put("/company/", dependencies=[Depends(verify_token)])
def update_company_settings(payload: dict):
    ...
    try:
        ...
        vat_percent = payload.get("vat_percent", 15.0)

        cursor.execute("""
            UPDATE company 
            SET name = %s, logo_path = %s, address = %s, vat_number = %s, contact_email = %s, contact_phone = %s, vat_percent = %s 
            WHERE id = 1
        """, (name, logo_path, address, vat_number, contact_email, contact_phone, vat_percent))
