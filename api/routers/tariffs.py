...
def api_create_tariff(payload: dict, current_user: dict = Depends(verify_token)):
    ...
    try:
        ...
        cursor.execute("SELECT id FROM tariffs WHERE property_id = %s AND meter_type = %s", (property_id, meter_type))
        if cursor.fetchone():
            raise HTTPException(status_code=400, detail=f"A {meter_type} tariff already exists for this property.")
...
def api_update_tariff(tariff_id: int, payload: dict, current_user: dict = Depends(verify_token)):
    ...
    try:
        ...
        if meter_type != original_meter_type:
            cursor.execute("SELECT id FROM tariffs WHERE property_id = %s AND meter_type = %s AND id != %s", (tariff_property_id, meter_type, tariff_id))
            if cursor.fetchone():
                raise HTTPException(status_code=400, detail=f"A {meter_type} tariff already exists for this property.")
...
def api_delete_tariff(tariff_id: int, current_user: dict = Depends(verify_token)):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT property_id FROM tariffs WHERE id = %s", (tariff_id,))
        tariff_data = cursor.fetchone()
        if not tariff_data:
            raise HTTPException(status_code=404, detail="Tariff not found.")
            
        enforce_property_access(current_user, tariff_data[0])

        # Check if any meters are using this tariff before deleting
        cursor.execute("SELECT COUNT(*) FROM meters WHERE tariff_id = %s", (tariff_id,))
        if cursor.fetchone()[0] > 0:
            raise HTTPException(status_code=400, detail="Cannot delete tariff. It is currently assigned to one or more meters.")

        cursor.execute("DELETE FROM tariffs WHERE id = %s", (tariff_id,))
        conn.commit()
        return {"status": "success", "message": f"Tariff {tariff_id} deleted successfully."}
    except Exception as e:
        conn.rollback()
        raise HTTPException(status_code=400, detail=str(e))
    finally:
